...
export async function loadApp<T extends ObjectType>(
  ...
  const { entry, name: appName } = app; // 用户配置的入口 和 名字
  ...
  // 会通过 entry 子应用的入口 拿到用户写的模板 html（包含js和css的）
  // template 是注释掉外链的 html
  // execScripts 代表的是用户引入的 js
  // 子应用的 publicPath
  ...
  // strictStyleIsolation 是否 shadowDOM
  ...
  // getAppWrapperGetter 如果是个 shadow ,需要拿到 shadow 里的 dom，shadow.root
  ...
  const useLooseSandbox = typeof sandbox === 'object' && !!sandbox.loose; // 快照沙箱，或者代理沙箱
  ...
  if (sandbox) {
    ...
    global = sandboxContainer.instance.proxy as typeof window; // 这里不再用 window 了而是用 proxy 中的对象作为后续的 window
    mountSandbox = sandboxContainer.mount; // 沙箱挂载，启动沙箱
    unmountSandbox = sandboxContainer.unmount; // 沙箱卸载，禁用沙箱
  }
  // 全局钩子，qiankun 中有环境变量: window.__powerBy__,window.publicPath
  const {
    beforeUnmount = [],
    afterUnmount = [],
    afterMount = [],
    ...
    const parcelConfig: ParcelConfigObject = {
      ...
      unmount: [
        ...
        async () => {
          ...
          offGlobalStateChange(appInstanceId); // 关闭监听
          // for gc
          appWrapperElement = null;
          syncAppWrapperElement2Sandbox(appWrapperElement);
        },
        async () => { // 应用卸载后完毕后，会把挂载的应用，表示卸载完成可以挂载新的应用
          if ((await validateSingularMode(singular, app)) && prevAppUnmountedDeferred) {
