...
export class AtService {
   ...
   */
  async sendMessageToMentionUsers(
    text: any,
    topicId: any,
    authorId: any,
    reply_id = null
  ) {
    let users = await this.userService.getUsersByNames(this.fetchUsers(text));

    users = users.filter((user: { _id: { equals: (arg0: any) => any } }) => {
      return !user._id.equals(authorId);
    });

    return Promise.all(
      users.map((user: { _id: any }) => {
        return this.messageService.sendAtMessage(
          user._id,
          authorId,
          topicId,
          reply_id
